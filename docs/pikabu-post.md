...
## Проблема 1: Видео приходит как `document`

Когда кто-то отправляет видео в Telegram без сжатия, оно приходит не как `Video`, а как `Document` с MIME-типом `video/mp4`. Важно сохранить выбор отправителя: обычное видео должно остаться видео, а отправленное файлом — скачиваемым файлом без повторного сжатия.

**Решение:** смотрим на исходное поле Telegram, а не только на MIME-тип. `Video` загружаем в MAX как видео, `Document` — как файл с исходным именем. Если имени нет, генерируем его из MIME-типа.
