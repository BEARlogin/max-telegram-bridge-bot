...
type TGMessage struct {
	...
	// ViaInlineBot — бот, через которого пользователь сам отправил inline-результат.
	// Это обычное пользовательское сообщение, а не сигнал спама.
	ViaInlineBot string
	// BotForward — сообщение переслано ОТ аккаунта-бота (forward_origin user с
	// is_bot). В отличие от via_bot это источник готового автоматического сообщения
	// и может использоваться антиспамом как структурный сигнал.
	BotForward      string
...
type RequestChatSpec struct {
	...
	ChatIsChannel bool   // true — выбрать канал, false — группу
...
type CommandScope struct {
	Type string // "", "all_private_chats", "all_group_chats", "all_chat_administrators"
}
