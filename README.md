...
5. По умолчанию посты идут в обе стороны. Измените направление: `/crosspost direction tg>max`
...
| Команда (MAX-бот, личка) | Описание |
|---------|----------|
| `/crosspost <TG_ID>` | Начать настройку кросспостинга (затем переслать пост из MAX-канала) |
| `/crosspost direction tg>max\|max>tg\|both` | Направление кросспостинга |
| `/uncrosspost` | Удалить кросспостинг |

Для получения TG ID канала — перешлите пост из канала в личку TG-бота.
