...
func (r *sqliteRepo) ListMessageRoutesByMax(maxChatID int64, maxMsgID string) []MessageRoute {
	rows, err := r.db.Query(`SELECT tg_chat_id,tg_msg_id,COALESCE(origin,'')
		FROM messages WHERE max_chat_id=? AND max_msg_id=? ORDER BY tg_chat_id,tg_msg_id`, maxChatID, maxMsgID)
	if err != nil {
		return nil
	}
	defer rows.Close()
...
func (r *sqliteRepo) RetainTgMessage(tgChatID int64, tgMsgID int, mediaGroupID string) error {
...
func (r *sqliteRepo) CleanOldMessages() {
	...
	// Clean author routes first while the corresponding message mapping is
	// still available for the pinned-message join.
